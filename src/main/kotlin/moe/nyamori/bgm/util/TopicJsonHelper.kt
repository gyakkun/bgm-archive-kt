...
class TopicJsonHelper(
    ...
    fun handleBlogTagAndRelatedSubject(topic: Topic) {
        ...
        if (!relatedSubjectIds.isNullOrEmpty()) {
            bgmDao.upsertBlogSubjectIdMapping(relatedSubjectIds.map { Pair(topic.id, it) })
        }
        if (!tags.isNullOrEmpty()) {
            bgmDao.upsertBlogTagMapping(tags.map { Pair(topic.id, it) })
        }
    }

    companion object {
        private val LOGGER = LoggerFactory.getLogger(TopicJsonHelper::class.java)
