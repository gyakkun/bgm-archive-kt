...
object BlogTopicParserR698 : Parser {
    private val LOGGER: Logger = LoggerFactory.getLogger(BlogTopicParserR698.javaClass)
    ...
    private fun extractRelatedSubjectIdList(relatedSubjectDivList: List<JXNode>): List<Int> {
        return relatedSubjectDivList.mapIndexed { idx, cardDiv ->
            ...
            val innerDiv = containerDiv.selOne("/div[contains(@class,\"inner\")]")
            val subjectAnchor = innerDiv.selOne("/p[1][contains(@class,\"title\")]/a")
            val subjectHref = subjectAnchor.asElement().attr("href")
    ...
    fun main(args: Array<String>) {
        val htmlStr = File("E:\\[ToBak]\\Desktop_Win10\\351610.html").readText(StandardCharsets.UTF_8)
        val some = BlogTopicParserR698.parseTopic(htmlStr,351610, SpaceType.BLOG)
