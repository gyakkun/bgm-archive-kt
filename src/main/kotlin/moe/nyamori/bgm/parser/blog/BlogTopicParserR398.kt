...
object BlogTopicParserR398 : Parser {
    ...
    override fun parseTopic(htmlFileString: String, topicId: Int, spaceType: SpaceType): Pair<Topic?, Boolean> {
        ...
        try {
            ...
            val blogCommentListDiv = body.selectFirst("div#comment_list") 
                ?: body.select("div.re_info").first()?.parent()?.parent()?.parent()?.let {
                    // Fallback to relative parent if no comment_list id
                    body.selectFirst("div#column_container > div > div > div > div:nth-child(4)")
                }
    ...
    private fun extractBlogInfo(
        blogId: Int,
        blogTitleH1: Element,
        blogPostDateSmall: Element?,
        blogTagDiv: Element?,
        blogRelatedSubjectUl: Element?
    ): Pair<Topic, User> {
    ...
    private fun getUidFromAvatarSrc(src: String): Int {
        return src.substring(src.lastIndexOf("/") + 1, src.lastIndexOf("jpg"))
            .toCharArray().filter { it.isDigit() }.joinToString("").let {
    ...
    internal fun extractCommentFromDiv(
        ...
        val reInfoSmall = reInfoDiv.selectFirst("small")!!
