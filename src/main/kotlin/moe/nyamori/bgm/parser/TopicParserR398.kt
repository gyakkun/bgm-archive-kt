...
object TopicParserR398 {
    private val LOGGER: Logger = LoggerFactory.getLogger(TopicParserR398.javaClass)
    ...
    fun parseTopic(htmlFileString: String, topicId: Int, spaceType: SpaceType): Pair<Topic?, Boolean> {
        try {
            ...
            when (spaceType) {
                ...
                else -> throw IllegalArgumentException("Type $spaceType is not support in this parser!")
            }
            ...
            val thisTopic = Topic(
                id = topicId,
                space = if (spaceType == SpaceType.GROUP) Group(
                    type = spaceType,
                    name = groupName,
